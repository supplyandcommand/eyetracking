...
We want to keep our own parameters but we need to know which condition (star vs. count). The star vs. count is in the Fixations_EL & is all we need to transfer. 

```{r}

Fixations$Trial <- as.numeric(Fixations$Trial)

#doesn't work because no rows match (obviously)
Fixations_fixed <- left_join(Fixations, Fixations_EL, by = c("ParticipantID", "Trial"))

#subsets 
Fixations_subset <- Fixations_EL %>%
...
# Plots for models / Results
1. how do we know when it is star-search & when it is counting?

```{r}

#Duration: We need to know which trial is which (also for the models I would assume?)
ggplot(Fixations, aes(x = Duration, y = CONDITION?)) + 
  geom_density_ridges(scale = 3)

#n_of_saccades: Still need to know condition/trial 
